(function($){'use strict';var _PLUGIN_='dotdotdot';var _VERSION_='3.0.5';if($[_PLUGIN_]&&$[_PLUGIN_].version>_VERSION_)
    {return;}
    ...
    {this.watch();}
    return this;};$[_PLUGIN_].version=_VERSION_;$[_PLUGIN_].uniqueId=0;$[_PLUGIN_].defaults={ellipsis:'\u2026 ',truncate:'word',tolerance:0,keep:null,watch:'window'};$[_PLUGIN_].prototype={init:function()
    {this.watchTimeout=null;this.watchInterval=null;this.uniqueId=$[_PLUGIN_].uniqueId++;this.originalContent=this.$dot.contents();this.originalStyle=this.$dot.attr('style')||'';this.maxHeight=this._getMaxHeight()+this.opts.tolerance;if(this.$dot.css('word-wrap')!=='break-word')
    {this.$dot.css('word-wrap','break-word');}
    if(this.$dot.css('white-space')==='nowrap')
    {this.$dot.css('white-space','normal');}},getInstance:function()
    ...
    {$e.remove();}});var isTruncated=this._truncateNode(this.$dot);this.$dot[isTruncated?'addClass':'removeClass'](_c.truncated);this.$inner.find('.'+_c.text).each(function()
    {$(this).replaceWith($(this).contents());});this.$inner.find('.'+_c.keep).removeClass(_c.keep);this.$inner.replaceWith(this.$inner.contents());this.$inner=null;return isTruncated;},restore:function()
    ...
    {return(this.$inner.innerHeight()<=this.maxHeight);},_addEllipsis:function(txt)
    {var remove=[' ','\u3000',',',';','.','!','?'];while($.inArray(txt.slice(-1),remove)>-1)
    {txt=txt.slice(0,-1);}
    txt+=this.opts.ellipsis;return txt;},_getMaxHeight:function()
    {var arr=['height','maxHeight'],hgh=0;for(var a=0;a<arr.length;a++)
    {var h=window.getComputedStyle(this.$dot[0])[arr[a]];if(h.slice(-2)=='px')
    {h=parseFloat(h);hgh=hgh?Math.min(hgh,h):h;}}
    switch(this.$dot.css('boxSizing'))
    {case 'padding-box':case 'border-box':var arr=['paddingTop','paddingBottom'];for(var a=0;a<arr.length;a++)
